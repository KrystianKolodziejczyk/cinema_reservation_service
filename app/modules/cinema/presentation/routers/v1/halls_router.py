...
from app.modules.cinema.presentation.schemas.responses import AddHallResponse
...
@router.post("/", status_code=status.HTTP_201_CREATED, response_model=AddHallResponse)
async def add_hall(
    body: AddHallRequest,
    ...
    hall_id = await service.add_hall(dto=dto, user_role=user_data["role"])
    return AddHallResponse(hall_id=hall_id)
